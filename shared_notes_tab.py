...
def create_shared_notes_tab(notebook):
    tab = ttk.Frame(notebook)  # 新しいタブ作成
    notebook.add(tab, text="shared_notes")  # "メモ" タブを追加

    tab.columnconfigure(0, weight=1)
    tab.columnconfigure(1, weight=1)

    text_widget = tk.Text(tab, wrap="word", font=("Helvetica", 12), bg="gray94", relief="flat", state="normal")
    text_widget.grid(row=1, column=0, columnspan=2, padx=10, pady=10, sticky="nsew")

    # マークダウンファイルを読み込んで表示する関数
    def load_markdown_file():
        filepath = "./memo/shared_memo.md"  # 同じ階層にあるファイル名を指定
        try:
